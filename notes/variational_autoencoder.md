...
>[!IMPORTANT]
> In generative models we are interested in learning the distribution of the data $p(x)$ and then generating new samples from it. We might also be interested in calculating the likelihood of the observed data $p(x)$. For a model with latent variables $z$ we can calculate the likelihood of the observed data $p(x)$ as:
...
Generative models aim to learn the distribution of the data $p(x)$, generate new samples from it, and calculate the likelihood of observed data.
...
This integral is intractable because the latent space is often very high-dimensional, sometimes with millions of possible states, and the integral has no simple analytical solution. The true posterior $p(z|x)$ is also intractable to compute for the same reasons.
